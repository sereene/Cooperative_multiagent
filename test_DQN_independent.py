...
def env_creator(config=None):
    ...
    # Independent 학습 시 MirrorObservation을 썼는지 안 썼는지에 따라 아래 줄 유지/삭제 결정 필요
    # 보통 Shared일 때 필수지만, Independent일 때도 썼다면 유지해야 함.
    env = MirrorObservationWrapper(env)

    return FixedParallelPettingZooEnv(env)

# [수정됨] Independent Policy Mapping
def policy_mapping_fn(agent_id, *args, **kwargs):
    # 에이전트 ID(paddle_0, paddle_1)를 그대로 정책 ID로 반환
    return agent_id
...
def run_test():
    ...
    # 로드된 정책 확인 (디버깅용)
    print("Loaded Policies:", algo.workers.local_worker().policy_map.keys())
    ...
    for i in range(NUM_EPISODES):
        ...
        while not done:
            ...
            for agent_id, agent_obs in obs.items():
                # 여기서 paddle_0 -> paddle_0 정책, paddle_1 -> paddle_1 정책을 호출함
                policy_id = policy_mapping_fn(agent_id)
                
                action = algo.compute_single_action(
                    observation=agent_obs,
                    policy_id=policy_id, # [중요] 해당 에이전트의 정책 사용
                    explore=False 
                )
